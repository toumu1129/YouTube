...
"""s027 の動画を組み立てる。
  ...
  python3 build.py            # 出力: s027.mp4（1080x1920 / 30fps / 音声なし）

【重要】各カットの秒数は、この場で決め打ちしない。
`s027-narration.srt`（tools/make_srt.py が音声の無音を実測して作ったもの）から逆算する。
理由は s020/build.py の冒頭コメント、および assets/s020/README.md を参照。

s027は静止画6枚（f1/f2/f3/f3b/f4/f5.jpg）の構成（動画素材は不使用）。
- f1.jpg（アップ）、f2.jpg（巣穴での群れ）、f5.jpg（アップ、締め）はWikimedia Commons
  および出どころ不明のクレジット不要サイトの実写（f1/f2はCC BY 2.0のため要クレジット表記）。
- f3.jpg（死亡率の比較グラフ。ハダカデバネズミ＝青の平坦線、普通のネズミ＝オレンジの
  上昇線を1枚のグラフにまとめ、線の端にそれぞれの実写風動物を配置）、
  f4.jpg（研究対象であることを示す光るマーカーを耳元に重ねたハダカデバネズミの写真。
  旧版の研究者の動画・写真から差し替え）、f3b.jpg（細胞とヒアルロン酸のオーバーレイ）は
  生成AI画像（プロンプトはこのチャットのやりとり参照）。1080x1920で生成済み。
"""
...
IMAGE_CARDS = [
    ("f1.jpg",  1,  2,  "image"),  # ①ハダカデバネズミは、ほとんど老化しません。
    ("f2.jpg",  3,  4,  "image"),  # ②寿命は30年ほどと…10倍以上です。
    ("f3.jpg",  5,  6,  "image"),  # ③前半 30年にわたる観察データでは…わかっています。
    ("f3b.jpg", 7,  8,  "image"),  # ③後半 体内で作られる…考えられています。
    ("f4.jpg",  9,  10, "image"),  # ④老化研究のモデル動物として…続けられています。
    ("f5.jpg",  11, 12, "image"),  # ⑤老いないネズミは…かもしれません。
]
...
CUTS, _BOUNDS = cuts_from_srt(HERE / "s027-narration.srt")
...
def main():
    print("カット秒数（s027-narration.srt から実測で算出）")
    ...
    subprocess.run([FF, "-y", "-loglevel", "error", "-f", "concat", "-safe", "0",
                    "-i", str(lst), "-c", "copy", str(HERE / "s027.mp4")], check=True)
    print("s027.mp4")
