...
s027は動画1本（f4.mp4、研究・観察のイメージ）＋静止画5枚（f1/f2/f3/f3b/f5.jpg）の構成。
- f1.jpg（アップ）、f2.jpg（巣穴での群れ）、f5.jpg（アップ、締め）はWikimedia Commons
  および出どころ不明のクレジット不要サイトの実写（f1/f2はCC BY 2.0のため要クレジット表記）。
- f4.mp4はPexelsの実写動画（ラボでノートPCを操作する研究者）を1080x1920に切り出し済み
  （旧版の顕微鏡映像から差し替え）。
- f3.jpg（死亡率のグラフのオーバーレイ）、f3b.jpg（細胞とヒアルロン酸のオーバーレイ）は
  ①②⑤の実写トーンに合わせ、実写のハダカデバネズミに図解を重ねる形で生成したAI画像
...
IMAGE_CARDS = [
    ...
    ("f4.mp4",  9,  10, "video"),  # ④老化研究のモデル動物として…続けられています。
