...
偏り、YouTube側のUI（タイトル・アカウント名・関連動画などの白文字）と被って
しまった。そのため全カットの書き出し後、被写体そのものを上へ平行移動する
後処理（reframe_up）を挟む。やり方：各カットの上端から y_off px を切り捨て、
残り crop_h px をそのままキャンバス上端に詰める（拡大はしない＝ズームによる
歪み・サイズ変化なし。単純な上シフト）。空いた下部 pad px は黒背景で埋める。

【重要】この黒帯は「動画を縮めて余白を作る」ためではなく、YouTube側が
白文字（タイトル・チャンネル名・関連動画）を重ねてくる帯の背景を黒くして
その文字を読みやすくする・テロップの配置もしやすくする、という狙い。
なので帯は大きくしすぎない（動画本体を大きく削らない）。REFRAME_PADは
その帯の実測サイズ相当の小さめの値にとどめる。

y_off は素材ごとに違う（被写体の画面内の高さが素材ごとに違うため）。
実際に書き出したフレームを見て、被写体と黒帯の境界に不自然な欠け方が
ないか確認すること。
"""
...
REFRAME_PAD = 300  # 下に空ける高さ(px)。黒背景で埋める（画面の約15.6%、UI文字帯相当）
# セグメント番号(1始まり) → 上端から切り捨てる高さ(px)。そのぶん被写体が上へ動く。
UPSHIFT_YOFF = {1: 200, 2: 200, 3: 300, 4: 250, 5: 60}
