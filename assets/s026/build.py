...
s026は動画1本（f1.mp4、タコのアップ）＋静止画4枚（f2/f3/f4/f5.jpg）の構成。
- f1.mp4はPexelsの実写動画から、目とサイフォンが安定して見える5-9秒区間を
  1080x1920に切り出し済み（追加のクロップ・スケールはしない）。
- f2.jpg（タコの全身）、f5.jpg（岩陰で休むタコ）はPexelsの実写。
- f3.jpg（3つの心臓のX線風オーバーレイ）、f4.jpg（ジェット推進と心臓）は
  ①②⑤の実写トーンに合わせ、フォトリアル指定で生成したAI画像
...
IMAGE_CARDS = [
    ...
    ("f3.jpg", 5,  9,  "image"),  # ③酸素を運ぶのに…必要になりました。
