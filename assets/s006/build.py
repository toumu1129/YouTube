...
"""s006 の動画を組み立てる。
  ...
  python3 build.py            # 出力: s006.mp4（1080x1920 / 30fps / 音声なし）
...
CUTS, _BOUNDS = cuts_from_srt(HERE / "s006-narration.srt")
...
def main():
    print("カット秒数（s006-narration.srt から実測で算出）")
    for src, dur, kind in CUTS:
        print(f"  {src:26}{dur:6.2f}s  ({kind})")
    ...
    subprocess.run([FF, "-y", "-loglevel", "error", "-f", "concat", "-safe", "0",
                    "-i", str(lst), "-c", "copy", str(HERE / "s006.mp4")], check=True)
    print("s006.mp4")
