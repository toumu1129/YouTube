...
- ①遡上するサケ（f1.jpg）、③後半の遡上シーン（f3b.jpg）は、ユーザーが
  無料素材サイトで見つけて提供した実写（クレジット不要とのこと。
  URLは失念とのことで素材表に記録できていない）。
- ②海のサケ、③前半の3段階図解、④の実験図解は、はっきり写った実写素材が
  見つからなかったため（濁った滝の遠景に小さく写る程度、または低解像度）、
...
IMAGE_CARDS = [
    ...
    ("f3b.jpg",      9,  10, "image"),  # ③後半 稚魚のころに…河口から上流へとたどっていきます。（実写）
