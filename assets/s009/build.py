...
s009は静止画5枚（f1/f2/f3/f3b/f4.jpg）＋動画1本（f5-river.mp4、川面）の構成。
①〜④はすべて、ユーザーが生成AI（フォトリアル指定）または無料素材サイトで
用意した実写・実写風の写真に差し替え済み（当初はgen_diagrams.pyでイラスト
生成していたが、映像全体のトーンを実写風に統一するため差し替えた）。
出典URL・生成方法は台本ファイルの「素材」欄を参照。1080x1920に
クロップ済みなので追加のスケールはしない。

【f4.jpgは仮】現在のf4.jpgは稚魚の写真で、これは③後半（f3b.jpg）に
移した。④（鼻をふさぐ実験の場面）用の画像は別プロンプトで生成依頼中。
届いたらf4.jpgを差し替えること（build.pyの構成自体は変更不要）。

⑤川面のみ実写動画（Pexels）。必要な秒数だけ先頭から使う。
"""
...
IMAGE_CARDS = [
    ...
    ("f3.jpg",       5,  8,  "image"),  # ③前半 覚えているのは…組成比だと考えられています。
    ("f3b.jpg",      9,  10, "image"),  # ③後半 稚魚のころに…河口から上流へとたどっていきます。
    ("f4.jpg",       11, 14, "image"),  # ④鼻をふさいだサケは…確かめられました。（仮画像。差し替え予定）
