...
"""s028 の動画を組み立てる。
  ...
  python3 build.py            # 出力: s028.mp4（1080x1920 / 30fps / 音声なし）

【重要】各カットの秒数は、この場で決め打ちしない。
`s028-narration.srt`（tools/make_srt.py が音声の無音を実測して作ったもの）から逆算する。
理由は s020/build.py の冒頭コメント、および assets/s020/README.md を参照。

s028は動画3本（f1/f2/f5.mp4）＋静止画2枚（f3/f4.jpg）の構成。
- f1.mp4（ゾウの全身、サバンナを歩く）、f2.mp4（別個体、丘の緑地を歩く。全身が
  フレームに収まる構図に差し替え済み）、f5.mp4（複数個体の脚のアップ）は
  Pexelsの実写動画（f2はもともと1080x1920のためクロップ不要）。
  ②は「走る」映像が見つからなかったため、通常の歩行映像で代用している。
- f3.jpg（脚の骨格構造のX線風オーバーレイ）、f4.jpg（着地の衝撃波オーバーレイ）は
  ①②⑤の実写トーンに合わせ、実写のゾウに図解を重ねる形で生成したAI画像
...
IMAGE_CARDS = [
    ("f1.mp4", 1,  2,  "video"),  # ①ゾウは、ジャンプができません。
    ("f2.mp4", 3,  4,  "video"),  # ②どんなに速く走っても…ありません。
    ("f3.jpg", 5,  7,  "image"),  # ③脚の骨がほぼ一直線に…していません。
    ("f4.jpg", 8,  9,  "image"),  # ④もし無理に跳んで…かかってしまいます。
    ("f5.mp4", 10, 12, "video"),  # ⑤ゾウの脚は…脚なのです。
]
...
CUTS, _BOUNDS = cuts_from_srt(HERE / "s028-narration.srt")
...
def main():
    print("カット秒数（s028-narration.srt から実測で算出）")
    ...
    subprocess.run([FF, "-y", "-loglevel", "error", "-f", "concat", "-safe", "0",
                    "-i", str(lst), "-c", "copy", str(HERE / "s028.mp4")], check=True)
    print("s028.mp4")
